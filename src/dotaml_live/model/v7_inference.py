...
from . import models as v7_models  # noqa: E402

# Legacy aliases — these used to point at the dotaml-turbo experiment dir. Retained
# so vendored modules importing them keep working; the val-parquet reads that relied
# on them are replaced by registry artifacts (see queries/artifacts.py).
EXP_DIR = paths.PKG_DIR
...
class V7Foundation:
    ...
    def __init__(self,
        ...
        # A model "version" is a registry directory holding model.pt + config.yaml
        # + item_vocab.json + served artifacts. Defaults to the promoted 'live' dir.
        self.model_dir = Path(model_dir) if model_dir is not None else paths.live_model_dir()
        if ckpt_path is None:
            ckpt_path = paths.model_pt(self.model_dir)
        if config_path is None:
            config_path = paths.config_yaml(self.model_dir)
        ...
        # Item vocab (shipped alongside the checkpoint in the registry)
        vocab_path = paths.item_vocab_json(self.model_dir)
